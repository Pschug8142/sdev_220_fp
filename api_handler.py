...
Ideally, we would pull the user registration data into this module to get their realm and character name. From there, this program would gather their character's info use it to fill out the keys in the nested
dictionary below. Once done, the dictionary would be converted into a json fill and pushed to the database to be displayed on the website for other user's to inspect the player character and their stats/gear.
'''
...
#The below lists are used as index equivalents to the dictionaries. They are specifically used further into the code when assigning the stat values to player gear.
...
# The variables client_ID and client_secret are given through the battlenet API registration. They are passed as a sort of username and password to gain an access token from the api service. These two variables
# are obtained through an account with blizzard.api.
client_ID = "a1c50b73455b43fcb6cd2581635e652b"
client_secret = "4XR19xXGeWAwBQGoY1OSJG7evdCQzQY1"

#Code variables that wouldn't make it into production. In this build, these variables can be changed to reflect a new realm and character name to pull new data. All info should be lowercase. If errors 
#occur with a new realm and/or character, try searching up the specific 'realm-slug' for your realm online. Blizzard is finnicky about the realm names.
test_char = 'terregoat'
test_realm = 'korialstrasz'
toon_name = test_char
toon_realm = test_realm

# 'access token' is requested. The resulting python dictionary returned from create_access_token is parsed and key matched to assign the returned access token to the variable 'access_token'
...
# There is data present that isn't required from the api request. Here, its being disposed of by setting the 'equipment_list' variable equal to the desired
...
while i in range(len(gear_list)):
     ...
     if x == 0:
     # The three if statements below are checking for specific item attributes. These attributes fall outside of the blizzard stat structure being parsed in the next while loop, so to be assigned
     # they are checked for here.
     ...
     while x in range(len(attribute_stats)): #
          ...
          attribute = attribute_stats[x] #x is being used to iterate through stats indices. This is necessary over a bulk assignment structure as not every stat type will be present on every item.
                                         # When a stat is matched to a value below, it is considered found and its corresponding stat value is assigned to its key pair compliment.
       
          
          #This block checks to see which stat is being assigned.
...
# produced dictionary. The goal here is to get rid of useless information.   
i = 0
while i in range(len(gear_list)):
     x = 0
...
#print(gear_json, '\n')
print(player_json) #This print only exists to show what output is generated by this module. It would not be here if fully implemented into production.
